...
//5（ctinfdefのサブセット）構造体（構造体各要素は可変文字列とする）
        ...
        */
            ...
            
        {
            ...
            //Rev23.10 不要のため削除 by長野 2015/10/05
        ...
        //Rev23.10 追加 by長野
        //シフトパラメータのセット
        public static void sftpar_set()
        {
            long det_sft_pix;	//検出器シフト量（画素）
            long inv_pix;	//無効画素（画素）検出器片側分
            long real_mainch;	//検出器横方向画素数（画素）無効画素含む
            long sft_wt_flt;	//ｳｴｲﾄを0.5にする部分の長さ（画素） 
            long cmp_length;	//検出器の重なり部分の長さ(画素) 無効画素含む

            TransImageControl.sft_par.Initialize();

            det_sft_pix = CTSettings.scancondpar.Data.det_sft_pix;	//検出器シフト量(画素)
            inv_pix = CTSettings.scancondpar.Data.inv_pix;			//無効画素(画素)検出器片側分
            real_mainch = CTSettings.scancondpar.Data.mainch[0] / (long)CTSettings.scancondpar.Data.h_mag[CTSettings.scansel.Data.binning]; 	//検出器横方向画素数(画素) 無効画素含む
            sft_wt_flt = CTSettings.scancondpar.Data.sft_wt_flt_pix;	//ｳｴｲﾄを0.5にする部分の長さ(画素)
            cmp_length = real_mainch - det_sft_pix;			//検出器の重なり部分の長さ(画素)

            if (sft_wt_flt > (cmp_length - (2 * inv_pix)))
                sft_wt_flt = cmp_length - (2 * inv_pix);
            
            TransImageControl.sft_par.mainch = (int)(real_mainch + det_sft_pix);
            TransImageControl.sft_par.wt_st1 = (int)(det_sft_pix + inv_pix);
            TransImageControl.sft_par.wt_end1 =(int)(det_sft_pix + (cmp_length - sft_wt_flt) / 2.0) - 1;
            TransImageControl.sft_par.wt_st2 = (int)(TransImageControl.sft_par.wt_end1 + sft_wt_flt + 1);
            TransImageControl.sft_par.wt_end2 = (int)(TransImageControl.sft_par.wt_st2 - inv_pix + (cmp_length - sft_wt_flt) / 2.0) - 1;
            TransImageControl.sft_par.wt[0] = (0.5 - 1.0) / (double)(TransImageControl.sft_par.wt_end1 - TransImageControl.sft_par.wt_st1 + 2);
            TransImageControl.sft_par.wt[1] = (1.0 - 0.5) / (double)(TransImageControl.sft_par.wt_end2 - TransImageControl.sft_par.wt_st2 + 2);
