...
//			temp = Val(txtValue.Text)
            ...
            int temp = (int)(d / (decimal)myDiscreteInterval) * (int)myDiscreteInterval;

			this.Value = modLibrary.CorrectInRange(temp, (int)this.Min, (int)this.Max);
		}
		...
		private void NumTextBox_Resize(object sender, EventArgs e)
		{
            ...
            panel1.SetBounds(lblColon.Right - 2, 0, lblUnit.Left - (lblColon.Right - 5), ClientRectangle.Height);

			Size size = panel1.ClientSize;

			if (!myIncDecButton)
			{
				size.Width += UpDownButtonWidth;
			}
			...
			nudValue.Location = new Point(0, (panel1.Height - nudValue.Height) / 2);
		}
