...
namespace CT30K
{
    ...
    public partial class frmMulti : Form
    {
        ...
        // frmMultiのインスタンス
        private static frmMulti _Instance = null;

        /// <summary>
        /// frmMultiのインスタンスを返す
        /// </summary>
        public static frmMulti Instance
        {
            get
            {
                if (_Instance == null || _Instance.IsDisposed)
                {
                    _Instance = new frmMulti();
                }
        ...
        public frmMulti()
        {
            ...
            lstImageFile.Font = new Font("ＭＳ Ｐゴシック", 12F, FontStyle.Regular, GraphicsUnit.Point, ((byte)(128)));
        ...
        private void cmdExe_Click(object sender, EventArgs e)
        {
            ...
            //マルチフレーム処理実行：成功した場合、保存ファイル名（候補）を設定
            SaveFileName = (ImgProc.CtMultiFrame(fileNameList) ? lstImageFile.List(lstImageFile.ListCount - 1) : "");
            ...
            {
                //メッセージ表示：マルチフレーム処理に失敗しました。
                string msg1 = StringTable.GetResString(StringTable.IDS_Processing, this.Text);
                string msg2 = StringTable.GetResString(StringTable.IDS_WentWrong, msg1);

                MessageBox.Show(msg2, Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                //処理画像を表示
        ...
        //履　　歴： v1.00  99/XX/XX   ????????      新規作成
        //*******************************************************************************
        private void frmMulti_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                //結果保存ダイアログ
                if (!frmImageSave.Instance.Dialog(SaveFileName, CTResources.LoadResString(10707)))    //ﾏﾙﾁ
                {
                    e.Cancel = true;
                }
            }
		...
		private void InitControls()
		{
			...
			//項目削除用のボタンの設定
            lstImageFile.DeleteButton = cmdDel;
            ...
            //最大表示枚数の表示
            lblMaxNum.Text = StringTable.GetResString(StringTable.IDS_FramesWithMax, lstImageFile.Max.ToString());
		}
    }
}
