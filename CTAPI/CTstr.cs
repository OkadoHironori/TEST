...
namespace CTAPI
{
    ...
    public class CTstr
    {
        ...
        public struct SCANINH
        {
            ...
            public int table_x_axis_move_acquire;//テーブルX軸移動収集 0:あり、1:なし Rev23.40 追加 by長野 2016/06/19 

            public int w_scan                   ;//Wスキャン 0:あり、1:なし  Rev25.00 追加 by長野 2016/06/19

            public int mbhc_phantomless         ;//Rev26.00 add ファントムレスBHC機能有無(0:有、1:無) by chouno 2016/12/27

            public int guide_mode               ;//Rev26.10 ガイドモードの有無(0:有、1:無) by井上 2017/12/12

            public int auto_correct_before_scan ;//Rev26.10 スキャン前に未完了の校正を実施の有無(0:有、1:無) by井上 2017/12/12

            public int auto_judge               ;//Rev26.10 校正結果の自動判定の有無(0:有、1:無) by井上 2017/12/12

            public int max_fdd1200              ;//Rev26.10 FDD最大値(0:1200, 1:1000) by井上 2017/12/12
        ...
        public struct INFDEF
        {
            ...
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            // Mod Start 2018/10/30 M.Oyama V26.40 Windows10対応
            //public FixedString8[] scan_area;/* ｽｷｬﾝ撮影領域表示文字		*/
            public FixedString16[] scan_area;/* ｽｷｬﾝ撮影領域表示文字		*/
            // Mod End 2018/10/30
            
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            // Mod Start 2018/08/20 M.Oyama 中国語対応
            //public FixedString8[] scano_area;/* ｽｷｬﾉ撮影領域表示文字		*/
            public FixedString16[] scano_area;/* ｽｷｬﾉ撮影領域表示文字		*/
            // Mod End 2018/08/20
            ...
            public FixedString16[] fc;/* ﾌｨﾙﾀ関数					*/
            // Mod End 2018/08/20
            ...
            public void Initialize()
            {
                ...
                scan_area = FixedString.CreateArray<FixedString16>(3);       // ｽｷｬﾝ撮影領域表示文字
                // Mod End 2018/10/30
                // Mod Start 2018/08/20 M.Oyama 中国語対応
                //scano_area = FixedString.CreateArray<FixedString8>(3);      // ｽｷｬﾉ撮影領域表示文字
                scano_area = FixedString.CreateArray<FixedString16>(3);      // ｽｷｬﾉ撮影領域表示文字
                // Mod End 2018/08/20
                ...
                fc = FixedString.CreateArray<FixedString16>(3);              // ﾌｨﾙﾀ関数
                // Mod End 2018/08/20
        ...
        public struct T20KINF
        {
            ...
            public FixedString8 version;/* ｿﾌﾄﾊﾞｰｼﾞｮﾝ情報				*/
            // Mod End 2018/08/20
            ...
            public int pki_fpd_type;//パーキンエルマー製FPD 型式 by chouno 2017/02/08

            public int ftable_type;//微調テーブルタイプ(0:内臓,1:試料テーブル上) by chouno 2019/02/06 Rev26.20
        ...
        public struct SCANCONDPAR
        {
            ...
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            public float[] ExObsCamPixSize;            //外観カメラ画像上の1画素サイズ([0]:非ズーム時,[1]:ズーム時,[2]未使用) //Rev25.00 コメント変更 by長野 2016/07/07 ([0]:ﾊｰﾌ、ﾌﾙ,[1]:ｵﾌｾｯﾄ,[2]ｼﾌﾄ)
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            public float[] ExObsCamPixSizeOnFTable;    //外観カメラ画像上の1画素サイズ(微調テーブル上)([0]:非ズーム時,[1]:ズーム時,[2]未使用) //Rev25.00 コメント変更 by長野 2016/07/07 ([0]:ﾊｰﾌ、ﾌﾙ,[1]:ｵﾌｾｯﾄ,[2]ｼﾌﾄ)
            //[MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            //Rev25.00 Wスキャン用に配列増やす SizeConst = 3 → 3*2 by長野 2016/07/07 
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = (3 * 2))]
            public int[] ExObsCamMaxArea;            //外観カメラ画像上で描画可能な円形ROIの最大直径(mm)([0]:非ズーム時,[1]:ズーム時,[2]未使用) //Rev25.00 コメント変更 by長野 2016/07/07 ([0]:ﾊｰﾌ、ﾌﾙ,[1]:ｵﾌｾｯﾄ,[2]ｼﾌﾄ)
            //<---

            public float opticalPathForCamera;  //外観カメラ光路(mm) Rev26.00 add by chouno 2017/01/16
            ...
            public void Initialize()
            {
                ...
                //Rev25.00 Wスキャン用に配列増やす by長野 2016/07/07
                ExObsCamMaxArea = new int[3 * 2];
        ...
        public struct MECHAPARA
        {
            ...
            public float min_fcd                  ;//最小FCD(オフセット無,仕様書上の値) Rev23.40 追加 by長野 2016/06/19
            public float min_fdd                  ;//最小FDD(オフセット無,仕様書上の値) Rec23.40 追加 by長野 2016/06/19
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 3)]
            public float[] sampleTableSize      ;//試料テーブルサイズ直径(mm) Rev26.00 add by chouno 2017/01/17

            public float largeTableRingWidth;//回転大テーブルリング幅(mm) Rev25.10 追加 by chouno 2017/09/12
            ...
            public void Initialize()
            {
                ...
                sampleTableSize = new float[3];     // 試料テーブルサイズ直径(mm) 
            }
            #endregion
        ...
        public struct SCANSEL
        {
            ...
            public int w_scan;                   //Rev25.00 Wスキャン(0:なし,1:あり) 追加 by長野 2016/07/05

            public int mbhc_phantomless;              //Rev26.00 ファントムレスBHC
            ...
            public float mbhc_phantomless_b;     //Rev26.00 ファントムレスBHC用係数２　by井上 2017/02/10
            public int mbhc_method;              //BHC手法の選択(0:ファントムレスBHC、1:従来BHC by井上 2017/04/12

            public int MultiImgNo;               //v26.42 by chouno 2021/07/20
        ...
        public struct IMAGEINFO
        {
            ...
            public int pki_fpd_type;            //PKI製FPD 型式 add by chouno 2017/02/07
            public float mbhc_phantomless_b;    //Rev26.00 ファントムレスBHC用係数２　by井上 2017/02/10
            public int mbhc_method;             //BHC手法の選択(0:ファントムレスBHC、1:従来BHC by井上 2017/04/12
        ...
        　　//19.00->(電S2)永井
            ...
            public int mbhc_phantomless; //ファントムレスBHC(0:無し,1:有り) Rev26:00 追加 by井上 2017/01/19
            public int mbhc_method;             //BHC手法の選択(0:ファントムレスBHC、1:従来BHC by井上 2017/04/12
