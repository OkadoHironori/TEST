...
namespace XrayCtrl
{
	...
	//
	//       <備考>
	//           制御方法は130kV制御機（L9181）と同じである為、プログラム修正・変更が発生した場合は、
	//           clsCtrlXrayL9181Sにも対応する必要がある。
	//
	//   Date        Version     Designed/Changed
	//   2003/05/28  2.00        (NSD)Shibui
	//
	//***********************************************************************************
	public class clsCtrlXrayL9421S : IDisposable
	{
...
#region CT30Kv19.13_64bit 化不要コメントアウト_完全版
/*
		private const string COMM_SETTINGS = "38400,n,8,1";					// ボーレート
*/
#endregion CT30Kv19.13_64bit 化不要コメントアウト_完全版
...
*/
		...
		//----- X線制御機から返されるデータ -----
		//private const string STAT_XRAY_ON				= "X-RAY ON";          // X線照射確認 X線ON
		//private const string STAT_XRAY_OFF			= "X-RAY OFF";         // X線照射確認 X線OFF
		...
		//private const string STAT_NORMAL				= "NORMAL";            // 過負荷保護機能 動作なし
		...
		//private const string STAT_WARMUP_ON			= "WARMUP ON";         // ウォーミングアップ中
		//private const string STAT_WARMUP_OFF			= "WARMUP OFF";        // ウォーミングアップ中ではない
		//private const string STAT_PREHEAT				= "PREHEAT";           // プリヒート中
		...
		//private const string STAT_WARMUP_YET			= "WARMUP YET";        // ウォーミングアップ待機中
		//private const string STAT_STANDBY				= "STANDBY";           // X線照射待機中
		//private const string STAT_SWS					= "WARMUP";            // ウォーミングアップのモードとステップ
		private const string STAT_WARMUP_END			= "WARMUP END";        // ウォーミングアップ終了
		//private const string STAT_SFC					= "FCS";               // 焦点
		//private const string STAT_SFC_1				= "FCS 1";             // 小焦点
		//private const string STAT_SFC_2				= "FCS 2";             // 中焦点
		//private const string STAT_SFC_3				= "FCS 3";             // 大焦点
		//private const string STAT_SAT					= "Auto Off Time";     // 自動X線停止機能確認
		private const string STAT_SER					= "ERR";               // 制御基板異常確認
		//private const string STAT_STC					= "STC";               // 一括状態確認
...
//		private int mintInputMode;
		...
		//
		// mcomCtrlXray_OnComm イベントハンドラで使用する固有の static フィールド
		//
		...
		//
		// mtmrCtrlXray_Timer イベントで使用する static フィールド
		//
		private	static bool mtmrCtrlXray_Timer_blnTmrFlg;
		private	static int mtmrCtrlXray_Timer_intSendPointer;
		private	static DateTime mtmrCtrlXray_Timer_varSendTime;			//送信タイムアウト用
		...
		//
		// AutoOff メソッドで使用する固有の static フィールド
		//
		...
		//
		// Avail メソッドで使用する固有の static フィールド
		//
...
/*
				//----- X線装置制御用フォームの設定 -----
				if (mfrmCtrlXray == null)
				{
					//2004-09-14 Shibui
//					Set mfrmCtrlXray = New FormCtrl
					mfrmCtrlXray = new frmXray();
					Load(mfrmCtrlXray)
*/
#endregion CT30Kv19.13_64bit 化不要コメントアウト_完全版
...
*/
					...
					//----- タイマ設定 -----
#region CT30Kv19.13_64bit 化不要コメントアウト_完全版
//					mtmrCtrlXray = mfrmCtrlXray.tmrXray;
#endregion CT30Kv19.13_64bit 化不要コメントアウト_完全版
...
//					gblnXrayOver = true;
                    ...
                    ////					ipX_type = 1			//X線ﾀｲﾌﾟ   //v14.13削除 07-11-19 byやまおか at現地
                    //ModuleCTRLM.ipX_type = 0;			//X線ﾀｲﾌﾟ   //v14.13追加 07-11-19 byやまおか at現地
                    ModuleCTRLM.ipX_type = modIniFiles.XRAY_TYPE_NO_90KV;
...
*/
		...
		/// <param name="e"></param>
		private void mcomCtrlXray_OnComm(object sender, SerialDataReceivedEventArgs e)
		{
			string strData;
			string strCommand;
...
*/
			...
			float sngDelayTime;					//追加2010/02/24(KSS)hata
...
*/
				    ...
				    //データ受信
				    int length = mcomCtrlXray.BytesToRead;
				    ...
				    mcomCtrlXray.Read(data, 0, length);
				    mstrRecvBuf += Encoding.ASCII.GetString(data);
				    ...
				    switch (mstrCommFlg)
				    {
					    ...
					    case CTRL_HIVOLT:
						    ...
						    if (mstrRecvBuf.StartsWith(STAT_ERROR))
						    {
					    ...
					    case CTRL_TYP:
						    ...
						    //追加2010/05/13(KS1)hata_L9421-02対応
						    //----- 型名確認 -----
						    modXray.gudtXrayP.strTYP = mstrRecvBuf;
						    break;
    ...
    //
					    ...
					    case CTRL_STC:
						    ...
						    int.TryParse(strData, ns, null, out iVal); 
...
Retry: 
			try
			{
				mcomCtrlXray.DiscardInBuffer();			//バッファクリア

                //変更2015/03/16hata
                // CTSﾗｲﾝｴﾗｰﾁｪｯｸ
                DateTime ntim = DateTime.Now;
                while (!mcomCtrlXray.CtsHolding)
                {
                    if (((DateTime.Now - ntim).TotalSeconds > 2000)) break;
                    Thread.Sleep(1);
                }
                
                mcomCtrlXray.Write(Buf + "\r");			//コマンド送信
...
//						Call XrayDisp	//X線情報表示
						...
						//----- ステータスモニタ -----
						switch (mtmrCtrlXray_Timer_intSendPointer)
						{
							case P_STX:  //X線照射確認
							...
							case P_CTRL: //コントロールコマンド
								...
								if (ModuleCTRLM.ipX_On == 1)
								{
									...
									{
										ModuleCTRLM.ipX_Watt = (float)(ModuleCTRLM.ipX_Volt / 1000000 * ModuleCTRLM.ipX_Amp * 1000);
									}
								}
								else
								{
...
//						Else
							...
							{
								case P_CTRL:
									mtmrCtrlXray_Timer_intSendPointer = P_SWS;	//ウォーミングアップステップ確認
									break;
								case P_SWS:
									mtmrCtrlXray_Timer_intSendPointer = P_SWE;
									break;
								case P_SWE:
									mtmrCtrlXray_Timer_intSendPointer = P_TYP;	//型名
									break;
								case P_TYP:
									mtmrCtrlXray_Timer_intSendPointer = P_STC;	//一括状態確認
									break;
								case P_STC:
									mtmrCtrlXray_Timer_intSendPointer = P_STT;	//動作管電圧管電流カソード値確認
									break;
								case P_STT:
									mtmrCtrlXray_Timer_intSendPointer = P_SVI;	//設定電圧値電流値確認
									break;
//2004-09-09 Shibui
//								Case P_SVI:     intSendPointer = P_CTRL  //コントロールコマンド
								case P_SVI:
									mtmrCtrlXray_Timer_intSendPointer = P_SER;	//異常
									break;
								case P_SER:
									mtmrCtrlXray_Timer_intSendPointer = P_CTRL;	//コントロールコマンド
									break;
								default:
									mtmrCtrlXray_Timer_intSendPointer = P_CTRL;
									mstrCommFlg = "";
									break;
							}
//						End If
...
//				With gudtXrayStatus1Value
					...
					//設定値変更監視
					if (Avail_intOldXrayVoltage != ModuleCTRLM.ipcndVolt ||
						Avail_intOldXrayCurent != ModuleCTRLM.ipcndAmp)
					{
