...
namespace XrayCtrl
{
	...
	//   2010/02/18  1.00        (KSS)hata
	//
	//***********************************************************************************
	internal class clsCtrlXrayL8601 : IDisposable
	{
...
*/
		...
		private ThreadEx mtmrCtrlXray = null;				// スレッド
		private SerialPort mcomCtrlXray = null;				// 通信

		private string mstrCommFlg = string.Empty;			// 通信ハンドシェイク用
		private int mintSovOver = 0;						// 過負荷保護機能状態
...
//		Private Const COMM_SETTINGS         As String = "9600,n,8,1"       ' ボーレート
        ...
        private bool RecieveSts = false;					//返信状態フラグ(True:正常、False:異常)
		private bool SendSts = false;						//送信状態フラグ(True:正常、False:異常)
		private bool LogWriteRFlg = false;
		private bool LogWriteSFlg = false;

		private const int COMM_RETRY = 3;					// 通信リトライ回数
		private string mstrRecvBuf = string.Empty;
...
//2003-11-18 Shibui
		private bool mblnCheckComm = false;					//通信確認用
...
//2003-11-18 Shibui
		...
		private int iWarmUPEND = 0;												// ウォーミングアップ終了ステータス（内部）

#region CT30Kv19.13_64bit 化不要コメントアウト_完全版
//		private int mintInputMode = 0;
#endregion CT30Kv19.13_64bit 化不要コメントアウト_完全版
		...
		//
		private static int mcomCtrlXray_OnComm_intErr1 = 0;
		private static int mcomCtrlXray_OnComm_intErr2 = 0;
		private static int mcomCtrlXray_OnComm_intErr3 = 0;
		private static int mcomCtrlXray_OnComm_intErr4 = 0;
		private static TimeSpan mcomCtrlXray_OnComm_sngTimCont1 = TimeSpan.Zero;
		private static TimeSpan mcomCtrlXray_OnComm_sngTimCont2 = TimeSpan.Zero;
		private static TimeSpan mcomCtrlXray_OnComm_sngTimCont3 = TimeSpan.Zero;
		private static TimeSpan mcomCtrlXray_OnComm_sngTimCont4 = TimeSpan.Zero;

		//
		// mtmrCtrlXray_Tick イベントで使用する static フィールド
		//
		private static bool mtmrCtrlXray_Tick_blnTmrFlg = false;
		private static int mtmrCtrlXray_Tick_intSendPointer = 0;
		private static DateTime mtmrCtrlXray_Tick_varSendTime = DateTime.MinValue;	//送信タイムアウト用 'v14.14変更 by 間々田 2008/02/20
		private static DateTime? mtmrCtrlXray_Tick_CommTimeOut = null;				//タイムアウト用
        //追加2015/04/03hata
		...
		//
		private static int AutoOff_intOldXrayOn = 0;
		private static int AutoOff_intTimeStart = 0;
		private static DateTime AutoOff_varTimXray = DateTime.MinValue;

		//
		// Avail メソッドで使用する static フィールド
		//
		private static int Avail_intOldXrayOnAvail = 0;
		private static int Avail_intXrayAvailOnRun = 0;
		private static int Avail_intXrayAvailInsideRun = 0;
		private static int Avail_intXrayAvailTime = 0;
		private static int Avail_intXrayAvailOk = 0;
		private static int Avail_intOldXrayVoltage = 0;
		private static int Avail_intOldXrayCurent = 0;
		private static DateTime Avail_varXrayAvailStartTime = DateTime.MinValue;

		/// <summary>
		/// 
		/// </summary>
		public clsCtrlXrayL8601()
		{
			lock(cCtrlm.gLock)
			{
				Class_Initialize();
...
*/
                    ...
                    //ModuleCTRLM.ipX_type = 4;		//X線ﾀｲﾌﾟ(L8601)
				    ...
				    switch (mstrCommFlg)
				    {
					    ...
					    case CTRL_DAG:
						    ...
						    if (strCommand == STAT_AGING)
						    {
							    ...
							    double d = 0;
							    double.TryParse(strData, out d);
							    modData.gintSWS = (int)d;
					    ...
					    case CTRL_STS:
						    ...
						    //焦点モードは固定
						    if (ModuleCTRLM.ifFocussize != 0)
						    {
							    ModuleCTRLM.ifFocussize = 0;
    //						ModuleCTRLM.ifFocussize = ModuleCTRLM.ifFocussize;
						    }
						    break;
					    ...
					    case CTRL_SER:
						    ...
						    if (strCommand == STAT_SER)
						    {
							    ...
							    if (char.IsNumber(strData, 0) == true)
							    {
							...
							case P_CTRL:	//コントロールコマンド
								...
								if (ModuleCTRLM.ipWarmup != 1)
								{
									...
									if (ModuleCTRLM.ifX_Amp != ModuleCTRLM.ipcndAmp)
									{
										...
										strData = Convert.ToString(ModuleCTRLM.ifX_Amp);		// 設定値取得(uA)
										if (ModuleCTRLM.ipX_On == modData.CTRL_ON)
										{
									...
									//----- フォーカスモード送信 -----
//									If (ifFocussize <> ipFocussize) Then
//										If ifFocussize < 0 Or ifFocussize > 3 Then
//											ifFocussize = 1
//										Else
//											strCommand = CTRL_MODE_FOCUS & CStr(ifFocussize + 1)
